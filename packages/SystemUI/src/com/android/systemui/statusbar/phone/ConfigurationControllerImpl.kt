...
constructor(@Assisted private val context: Context) :
    ...
    override fun notifyThemeChanged() {
        ...
        listeners.filterForEach({ synchronized(this.listeners) { this.listeners.contains(it) } }) { it.onThemeChanged() }
    }

    override fun dispatchOnMovedToDisplay(newDisplayId: Int, newConfiguration: Configuration) {
        val listeners = synchronized(this.listeners) { ArrayList(this.listeners) }
        listeners.filterForEach({ synchronized(this.listeners) { this.listeners.contains(it) } }) {
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
        // Avoid concurrent modification exception
        val listeners = synchronized(this.listeners) { ArrayList(this.listeners) }
        listeners.filterForEach({ synchronized(this.listeners) { this.listeners.contains(it) } }) { it.onConfigChanged(newConfig) }
        ...
        if (maxBounds != this.maxBounds) {
            ...
            listeners.filterForEach({ synchronized(this.listeners) { this.listeners.contains(it) } }) { it.onMaxBoundsChanged() }
        }

        val localeList = newConfig.locales
        if (localeList != this.localeList) {
            this.localeList = localeList
            listeners.filterForEach({ synchronized(this.listeners) { this.listeners.contains(it) } }) { it.onLocaleListChanged() }
        }
        ...
        if (uiModeChanged) {
            ...
            this.uiMode = uiMode
            listeners.filterForEach({ synchronized(this.listeners) { this.listeners.contains(it) } }) { it.onUiModeChanged() }
        }
