...
class PrivacyConfig @Inject constructor(
    ...
    init {
        ...
        secureSettings.registerContentObserverForUser(
            Settings.Secure.LOCATION_PRIVACY_INDICATOR,
            settingsObserver, UserHandle.USER_CURRENT
        )
        secureSettings.registerContentObserverForUser(
            Settings.Secure.CAMERA_PRIVACY_INDICATOR,
            settingsObserver, UserHandle.USER_CURRENT
        )
        secureSettings.registerContentObserverForUser(
            Settings.Secure.PROJECTION_PRIVACY_INDICATOR,
            settingsObserver, UserHandle.USER_CURRENT
        )
    }
    ...
    private fun isMicCameraEnabled(): Boolean {
        return  secureSettings.getIntForUser(
            Settings.Secure.CAMERA_PRIVACY_INDICATOR, 1, UserHandle.USER_CURRENT) == 1
    }

    private fun isLocationEnabled(): Boolean {
        return secureSettings.getIntForUser(
            Settings.Secure.LOCATION_PRIVACY_INDICATOR, 1, UserHandle.USER_CURRENT) == 1
    }

    private fun isMediaProjectionEnabled(): Boolean {
        return secureSettings.getIntForUser(
            Settings.Secure.PROJECTION_PRIVACY_INDICATOR, 1, UserHandle.USER_CURRENT) == 1
    }
