...
public class TriStateUiControllerImpl implements TriStateUiController,
    ...
    private boolean mAlertSliderNotification = true;
    ...
    private final BroadcastReceiver mSliderStateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (mIntentActionSupported && action.equals(mIntentAction)) {
                ...
                if (extras != null) {
                    mPosition = extras.getInt(EXTRA_SLIDER_POSITION, -1);
                    mPositionValue = extras.getInt(EXTRA_SLIDER_POSITION_VALUE, -1);
                }
    ...
    public TriStateUiControllerImpl(
        ...
        mContext.registerReceiver(mSliderStateReceiver, filter, Context.RECEIVER_EXPORTED);
    }
            ...
            if (res != null) {
                ...
                switch (mOrientationType) {
                    case ROTATION_90:
                        ...
                        } else if (mTriStateMode == RINGER_MODE_SILENT || mTriStateMode == MODE_SILENT) {
                            positionX = res.getDimensionPixelSize(R.dimen.tri_state_up_dialog_position_l);
                        } else if (mTriStateMode == RINGER_MODE_VIBRATE || mTriStateMode == MODE_VIBRATE) {
                            positionX = res.getDimensionPixelSize(R.dimen.tri_state_middle_dialog_position_l);
                        } else if (mTriStateMode == RINGER_MODE_NORMAL || mTriStateMode == MODE_RING || mTriStateMode == MODE_NONE) {
                            positionX = res.getDimensionPixelSize(R.dimen.tri_state_down_dialog_position_l);
                        }
    ...
    private void applyRingerModeFromPositionValue(int positionValue) {
        ...
        if (targetRingerMode != -1 && am.getRingerModeInternal() != targetRingerMode) {
            am.setRingerModeInternal(targetRingerMode);
        }
    }
    ...
    private void handleStateChanged() {
        ...
        if (mIntentActionSupported && mPositionValue != -1) {
            applyRingerModeFromPositionValue(mPositionValue);
        ...
        if (stateChanged || !mShowing) {
            updateTriStateLayout();
            if (mListener != null) {
                mListener.onTriStateUserActivity();
            }
            mHandler.sendEmptyMessageDelayed(MSG_DIALOG_SHOW, DIALOG_DELAY);
        }
    }
