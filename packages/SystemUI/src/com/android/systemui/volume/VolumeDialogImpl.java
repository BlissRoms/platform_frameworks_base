...
public class VolumeDialogImpl implements VolumeDialog,
        ...
        mDialog.setOnShowListener(dialog -> {
            ...
            if (!shouldSlideInVolumeTray()) {
                mDialogView.setTranslationX((
                        getTranslationForPanelLocation() * mDialogView.getWidth() / 2.0f)*(isAudioPanelOnLeftSide() ? -1 : 1);
            }
            mDialogView.setAlpha(0);
            mDialogView.animate()
                    .alpha(1)
    ...
    protected void dismissH(int reason) {
        ...
        if (!shouldSlideInVolumeTray()) {
            animator.translationX((getTranslationForPanelLocation() * mDialogView.getWidth() / 2.0f)*(isAudioPanelOnLeftSide() ? -1 : 1));
        }
        animator.start();
