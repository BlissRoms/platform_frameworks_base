...
public final class VolumeDialogManager {
    ...
    @Inject
    public VolumeDialogManager(Context context, InjectionInflationController injectionInflater,
    ...
    @VisibleForTesting
    VolumeDialogManager(Context context, InjectionInflationController injectionInflater,
            PluginManager pluginManager, SysuiColorExtractor colorExtractor,
    ...
    private void reload() {
        ...
        mListeners.forEach((listener, dialogs) -> {
            ...
            if (Looper.myLooper() == Looper.getMainLooper()) {
                listener.onDialogChanged(dialog instanceof AospVolumeDialogImpl ? null : dialog);
            } else {
                mMainHandler.post(() -> listener.onDialogChanged(
                        dialog instanceof AospVolumeDialogImpl ? null : dialog));
            }
        });
    }
    ...
    private final class AvailablePlugins implements PluginListener<VolumeDialog> {
        ...
        void addVolumeDialog(VolumeDialog plugin) {
            ...
            mVolumeInfo.add(VolumeInfo.builder()
        ...
        private VolumeDialog getVolumeDialog() {
            ...
            final String name = "";
                    /*mSettingsWrapper.getLockScreenCustomClockFace(
                    mCurrentUserObservable.getCurrentUser().getValue());*/
            if (name != null) {
                plugin = mVolume.get(name);
            }
