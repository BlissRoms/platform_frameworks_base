...
public class SettingsWrapper {
    ...
    private static final String VOLUME_DIALOG_SETTING = Settings.Secure.CUSTOM_VOLUME_DIALOG;
    private static final String DOCKED_VOLUME_DIALOG = Settings.Secure.DOCKED_VOLUME_DIALOG;
     ...
     */
    String getCustomVolumeDialog(int userId) {
        return decode(
                Settings.Secure.getStringForUser(mContentResolver, VOLUME_DIALOG_SETTING, userId),
                userId);
    }
     ...
     * @param userId ID of the user.
     */
    String getDockedVolumeDialog(int userId) {
        return Settings.Secure.getStringForUser(mContentResolver, DOCKED_VOLUME_DIALOG, userId);
    }
    ...
    String decode(@Nullable String value, int userId) {
        ...
        try {
            return json.getString(VOLUME_FIELD);
        } catch (JSONException ex) {
    ...
    private static final class Migrator implements Migration {
        ...
        public void migrate(String value, int userId) {
            try {
                ...
                json.put(VOLUME_FIELD, value);
                Settings.Secure.putStringForUser(mContentResolver, VOLUME_DIALOG_SETTING,
                        json.toString(),
                        userId);
            } catch (JSONException ex) {
