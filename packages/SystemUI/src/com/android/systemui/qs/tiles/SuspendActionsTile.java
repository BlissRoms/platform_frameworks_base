...
public class SuspendActionsTile extends QSTileImpl<BooleanState> {
    ...
    public void handleLongClick() {
        Intent screenstate = new Intent(mContext,
               com.android.systemui.screenstate.ScreenStateService.class);
        if (isSuspendActionsEnabled()) {
            Settings.System.putIntForUser(mContext.getContentResolver(),
                Settings.System.START_SCREEN_STATE_SERVICE, 0, UserHandle.USER_CURRENT);
            mContext.stopService(screenstate);
        } else {
            Settings.System.putIntForUser(mContext.getContentResolver(),
                Settings.System.START_SCREEN_STATE_SERVICE, 1, UserHandle.USER_CURRENT);
